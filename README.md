...
| MUSCLE 的任务 | 对一张医学图像进行分类，不是定位或分割病灶 |
| ---- | ---- |
| 关于多尺度 | 浅层保留纹理和细节，深层表达更强的语义；病灶大小和分布不同，单一尺度可能不够 |
| MAFC 的作用 | 把不同阶段、不同尺寸的特征从 C/H/W 三个方向压缩成可融合的表示 |
| 证据 | 模型对各类别给出的非负支持量；证据总量越少，整体不确定性越高 |
| 训练的两个阶段 | 先得到可用的骨干网络，再冻结它并训练多尺度证据融合模块 |
...
原论文的尺度消融、同一团队的后续工作以及本仓库的短轮观察都说明：不同阶段的证据质量存在明显差异，浅层高不确定性信息有时会削弱深层有效信息。基于这一现象，我将一个后续研究问题概括为“尺度可靠性折扣”——在聚合前同时参考尺度自身的不确定性和尺度间一致性，降低低可靠性证据的影响，把人工选择阶段转化为针对每个样本的自适应可信尺度选择。

这仍是尚未实现和验证的研究设想，不是本仓库已经取得的性能改进。问题依据、与作者后续工作的关系、可行性和风险见[专题页：从固定聚合到可信尺度选择](docs/scale-reliability-improvement.md)。
